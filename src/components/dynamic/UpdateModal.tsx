...
type Input = {
    ...
    disabled?: boolean
    value?: any // Added value property
...
interface Props {
    ...
    title: string
    inputs: Input[]
    updateItem: (item: Record<string, any>) => void
...
const UpdateModal = ({
    ...
    const renderInput = (input: Input) => {
        ...
        const value = input.value !== undefined ? input.value : getValue(input)
        const onChange = input.onChange ? input.onChange : (e: any) => handleInputChange(input.name, e.target.value)
            ...
            case 'rte':
                return (
                    <div className={`p-2 ${input.width}`} key={input.name}>
                        ...
                        <RichTextEditor
                            content={value}
                            onChange={(html) => handleInputChange(input.name, html)}
                        />
                        {renderError(error)}
                    </div>
                )
